...
class AppStore extends Component {
  ...
  render() {
    ...
    const filterData = this.getFilteredProjects()
    const filterResults = filterData.filter(eachRe =>
      eachRe.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div className="bg-container">
        ...
        <ul className="ul2">
          {filterResults.map(eachApp => (
            <AppItem appItemDetails={eachApp} key={eachApp.id} />
          ))}
        </ul>
      </div>
    )
